...
class DatabaseManager:
    def __init__(
        self, sqlite_path: str = "nodus_local.db", qdrant_path: str = "nodus_vectors"
    ) -> None:
    ...
    async def save_episode(
        self, episode_id: str, timestamp: str, event_type: str, summary: str, context: str
    ) -> None:
        """Saves an episode to SQLite."""
        async with aiosqlite.connect(self.sqlite_path) as db:
            await db.execute(
                "INSERT INTO episodes (id, timestamp, event_type, summary, context) "
                "VALUES (?, ?, ?, ?, ?)",
